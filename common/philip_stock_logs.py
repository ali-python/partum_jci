...
from philip_inventory.models import StockOut


class PhilipDailyStockLogs(ListView):
    model = StockOut
    template_name = 'philip_inventory/stock/daily_stock_logs.html'
    paginate_by = 200
    is_paginated = True

    def __init__(self, *args, **kwargs):
        super(PhilipDailyStockLogs, self).__init__(*args, **kwargs)
        self.logs_date = ''
        self.today_date = ''
    ...
    def get_context_data(self, **kwargs):
        context = super(PhilipDailyStockLogs, self).get_context_data(**kwargs)
...
class PhilipMonthlyStockLogs(ListView):
    model = StockOut
    template_name = 'philip_inventory/stock/monthly_stock_logs.html'
    paginate_by = 200
    is_paginated = True

    def __init__(self, *args, **kwargs):
        super(PhilipMonthlyStockLogs, self).__init__(*args, **kwargs)
        self.logs_month = ''
        self.current_month = ''
        self.year = ''
    ...
    def get_context_data(self, **kwargs):
        context = super(PhilipMonthlyStockLogs, self).get_context_data(**kwargs)
        queryset = self.get_queryset()
        if queryset:
            total = queryset.aggregate(Sum('sale_price'))
            total = total.get('sale_price__sum') or 0
